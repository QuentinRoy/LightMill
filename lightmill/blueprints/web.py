...
@web_blueprint.route("/")
def index():
    return render_template("experiments_list.jinja", experiments=Experiment.query.all())
...
def experiment(experiment):
    ...
    return render_template(
        ...
        completed_nb=len([r for r in filter(lambda e: e[1] == e[2], run_statuses)]),
...
def generate_trial_csv(experiment):
    def generate():
        ...
        for [
            ...
            if current_record is not None and not (
                block_number == current_block_number and current_run_id == run_id
            ):
                current_block_factors = {}
            # Case we changed trial
            if current_record is not None and not (
                trial_number == current_trial_number
                and block_number == current_block_number
            ...
            if not current_record:
                ...
                current_measured_block_num = (
                    0
                    if not current_measured_block_num and not practice
                    else current_measured_block_num
                    if practice
                ...
                current_record = {
                    "trial": {
                        "server_completion_date": (
                            str(convert_date(completion_date))
                            if completion_date
                            else ""
                        ),
...
def run_results(experiment, run):
    ...
    trial_measures = sorted(
        (measure for measure in experiment.measures.values() if measure.trial_level),
        key=lambda x: x.id,
    )
    factor_values_names = dict(
        db.session.query(FactorValue.id, FactorValue.name)
        .join(FactorValue.factor)
        .filter(Factor.experiment == experiment)
        .filter(FactorValue.name.isnot(None))
    )
    return render_template(
        "results_static.jinja",
...
def events(experiment, run, block, trial):
    event_measures = sorted(
        (measure for measure in experiment.measures.values() if measure.event_level),
        key=lambda x: x.id,
    )
    return render_template(
        "events.jinja",
